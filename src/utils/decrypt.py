...
class ChessDecrypter:
    """Decrypt chess game PGN files back into a binary file.

    Reads the PGN files produced by ChessEncrypter and reconstructs the
    original bytes by decoding each data move back into a bit. Bit 0 is
    decoded from the first possible move and bit 1 from the second.
    """
    ...
    def __init__(
        ...
        """Initialize the decrypter with an input directory and output file."""
    ...
    def decrypt(self):
        """Decrypt all PGN files in the input directory to the output file."""
    ...
    def process_move(self, move):
        """Decode a single move into a bit and update the board state."""
